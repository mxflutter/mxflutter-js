//  MXFlutterFramework
//  Copyright (C) 2021 THL A29 Limited, a Tencent company.  All rights reserved.
//  Use of this source code is governed by a BSD-style license that can be
//  found in the LICENSE file.

import { Key } from '../foundation/key';
import { Element$, BuildContext$, StatelessWidget$, Widget } from './framework';
import { MXDartClass, MXMirrorWidget, MXWidget } from '@mxflutter/mxflutter-base';
abstract class Notification extends MXDartClass {}
export { Notification };
class NotificationListener extends StatelessWidget$ {
  child: MXWidget;
  onNotification: any;
  public constructor(namedParameters: { key?: Key; child?: MXWidget; onNotification?: any } = {}) {
    super();
    this.key = namedParameters.key;
    this.child = namedParameters.child;
    this.onNotification = namedParameters.onNotification;
    this.className = 'NotificationListener';
  }
}
export { NotificationListener };
class LayoutChangedNotification extends Notification {}
export { LayoutChangedNotification };
