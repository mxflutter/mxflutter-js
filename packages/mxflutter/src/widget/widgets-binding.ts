...
export class WidgetsBinding extends MXLifecycleDartClass {
  ...
  private flutterCallNavigatorPushWithName(args: { widgetName: string; widgetID: string; flutterPushParams: string }) {
  ...
  private navigatorPushWithName(widgetName: string, flutterHostWidgetID: string, flutterPushParams: Object, args: any) {
    // 调用到用户代码，根据widgetName创建JSWidget
    // TODO: TS Widget  let widget: MXJSBaseWidget
    const widget: MXJSBaseWidget = this.jsFlutterApp.createJSWidgetWithName(widgetName, flutterPushParams);
    if (!widget) {
