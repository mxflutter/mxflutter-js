...
import { MXJSHostStatefulWapperWidget } from './widget';
...
export class WidgetsBinding extends MXLifecycleDartClass {
    ...
    flutterCallNavigatorPushWithName(args) {
        ...
        this.navigatorPushWithName(widgetName, flutterHostWidgetID, flutterPushParams, args);
    }
    // Flutter通过MXJSFlutter.getInstance().navigatorPushWithName("JSWidgetHomePage",...);push页面时,会调用到此函数
    navigatorPushWithName(widgetName, flutterHostWidgetID, flutterPushParams, args) {
        // 调用到用户代码，根据widgetName创建JSWidget
        // TODO: TS Widget  let widget: MXJSBaseWidget
        const widget = this.jsFlutterApp.createJSWidgetWithName(widgetName, flutterPushParams);
        if (!widget) {
    ...
    flutterCallRefreshLazyWidget(args) {
        ...
        if (element) {
            ...
            MXJSLog.error(`WidgetsBinding::flutterCallRefreshLazyWidget error: element type:(${typeof element}) widgetID: ${widgetID}} `);
        }
        this.rootMgrElement.debugPrintElementTree();
    }
    // flutter -> js
    ...
    flutterCallOnBuildEnd(args) {
        ...
        if (element) {
            ...
            MXJSLog.error(`WidgetsBinding::onBuildEnd error: element == null widgetID: ${widgetID} buildWidgetDataSeq: ${buildWidgetDataSeq}`);
        }
    }
    ...
    flutterCallOnFirstFrameEnd(args) {
        ...
        if (element) {
            ...
            MXJSLog.error(`WidgetsBinding::onFirstFrameEnd error: element == null widgetID: ${widgetID} buildWidgetDataSeq: ${args.buildSeq}`);
        }
    }
